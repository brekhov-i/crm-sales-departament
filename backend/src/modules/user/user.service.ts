...
import { LoginBody, RegistrationBody, RoleBody } from "./user.types";
...
export class UserService {
  ...
  async getUserById(id: string) {
    ...
    if (!user) throw new HttpException(`Пользователь с id ${id}`, HttpStatus.NOT_FOUND)
  ...
  async login({ email, password }: LoginBody) {
    ...
    if (!candidate) throw new HttpException('Ошибка в email или пароле. Повторите попытку', HttpStatus.UNAUTHORIZED)

    const isValid = compareSync(password, candidate.password)

    if (!isValid) throw new HttpException('Ошибка в email или пароле. Повторите попытку', HttpStatus.UNAUTHORIZED)
  ...
  async registration(body: RegistrationBody) {
    const candidate = await this.userModel.findOne({ where: { email: body.email } })

    if (candidate) throw new HttpException('Пользователь уже существует', HttpStatus.CONFLICT);
  ...
  async createRole(body: RoleBody) {
    const candidate = await this.roleModel.findOne({ where: { name: body.name } })

    if (candidate) throw new HttpException('Такая роль уже существует', HttpStatus.CONFLICT)
